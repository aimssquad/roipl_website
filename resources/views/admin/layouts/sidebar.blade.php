<!-- resources/views/admin/layouts/sidebar.blade.php -->
<aside id="sidebar" class="sidebar">
    <ul class="sidebar-nav" id="sidebar-nav">

        <!-- Dashboard Nav -->
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'dashboard') collapsed @endif" href="{{ route('admin.dashboard') }}">
                <i class="bi bi-grid"></i>
                <span>Dashboard</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'roles') collapsed @endif" href="{{ route('admin.roles.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Role</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'users') collapsed @endif" href="{{ route('admin.users.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Users</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'permissions') collapsed @endif" href="{{ route('admin.permissions.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Permission</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'abouts') collapsed @endif" href="{{ route('admin.abouts.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Our Story</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'announcements') collapsed @endif" href="{{ route('admin.announcements.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Announcement</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'brands') collapsed @endif" href="{{ route('admin.brands.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Brand</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'brandlogos') collapsed @endif" href="{{ route('admin.brandlogos.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Brand Logo</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'events') collapsed @endif" href="{{ route('admin.events.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Life at ROIPL</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'events') collapsed @endif" href="{{ route('admin.visionnaires.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Visionnaire</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'teams') collapsed @endif" href="{{ route('admin.teams.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Teams</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'awards') collapsed @endif" href="{{ route('admin.awards.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Awards</span>
            </a>
        </li>

         <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'jobs') collapsed @endif" href="{{ route('admin.jobs.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Job Post</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'contacts') collapsed @endif" href="{{ route('admin.contacts.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Contacts</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'careers') collapsed @endif" href="{{ route('admin.careers.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Careers</span>
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link @if(Request::segment(2) != 'pages') collapsed @endif" href="{{ route('admin.pages.index') }}">
                <i class="bi bi-card-list"></i>
                <span>Pages</span>
            </a>
        </li>



    </ul>
</aside>
