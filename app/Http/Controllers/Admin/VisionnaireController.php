...
class VisionnaireController extends Controller
{
    ...
    public function __construct(){
        $this->prefix = 'admin.visionnaires.';
        // parent::__construct('Role');
    }
    public function index()
    {
        $datas = Visionnaire::all();
    ...
    public function store(Request $request)
    {
        ...
        // Create the event
        $event = Visionnaire::create($validated);

        // Handle single image upload
        if ($request->hasFile('image')) {
            $imagePath = $request->file('image')->store('visionnaires', 'public');
    ...
    {
        $data = Visionnaire::findOrFail($id);
